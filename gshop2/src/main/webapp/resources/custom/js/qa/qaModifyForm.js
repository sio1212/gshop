...
$(document).ready(function(){
	$.ajax({
		...
		success : function(data){
			...
			if($('#productid').val()!=''){
				var getImgOn='<img style="height:100px;width:90px;" src="/storage/showProduct.do?product_name_image='+$('#productid').val()+'">';				
			}
	...
	if($('#qa_subject').val()==''){ 
		...
		$.ajax({
			...
			success : function(){
				...
				window.location='/board/qa/qaView.do?qa_seq='+$('#qa_seq').val()+'&pg='+$('#pg').val();}
		});
	}
});
