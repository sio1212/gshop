...
function preOrderEvent(){
		...
		$.ajax({
			type: 'post',
			url : '/trading/addCart.do',
			data: {'product_name_no':$('#product_name_no').val(),
					'cart_qty':$('#cart_qty').val()},
			success: function(){
				var seeCart = confirm('현재 품목을 장바구니에 담았습니다. 장바구니를 확인하러 가시겠습니까?');
				if(seeCart){
					window.location='/trading/userCart.do';
				}//if
				else{
					alert('쇼핑목록으로 돌아갑니다.');
					window.location='/product/categories.do?product_category_name=ALL';}//else
				}

			});
}

function preOrderEvents(){
...
$('#writeQABtn').click(function(){
	window.location='/board/qa/qaWriteForm.do?productID='+$('#productID').val();
});
$('#writeReviewBtn').click(function(){
	window.location='/board/review/reviewWriteForm.do?productID='+$('#productID').val();	
});
