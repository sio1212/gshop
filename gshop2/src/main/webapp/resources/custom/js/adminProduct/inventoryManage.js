...
function getInventory(data){
	...
	$('#inventoryTable').on('click','#subjectA',function(){
			...
			var loginPop = window.open('/admin/product/inventoryModify.do?productID='+productID+'&pg='+$('#pg').val(),'재고수정','width=565,height=435,resizable=no');
...
$(document).ready(function(){
	$.ajax({
		type : 'post',
		url : '/admin/product/inventoryList.do',
		data : 'pg='+$('#pg').val(),
		dataType : 'json',
...
function productPaging(pg){
	location.href='/admin/product/inventoryManage.do?pg='+pg;
}
