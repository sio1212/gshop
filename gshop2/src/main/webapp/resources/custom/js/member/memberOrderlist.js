...
$(document).ready(function(){
	$.ajax({
		type: 'get',
		url : '/member/getOrderList.do',
		dataType: 'json',
		success: function(data){
			$('#orderListTable tr:gt(0)').empty();
			
			if(data.orderList.length==0||data.orderList==null){
				$('#orderListTable').append('<h5 style="color:red;font-weight:bold;text-decoration:underline;>주문 내역이 존재하지 않습니다</h5>');}
			else{
				$.each(data.orderList,function(index,items){
					...
					var str = new StringBuilder();
						...
						str.Append('<a class="productViewA" href="/trading/orderView.do?order_no='+items.order_no+'">');
					...
					$('<tr/>',{
						...
						html: productList						
								...
								style : 'border:1px solid #A48654;color:#D26E6E;font-weight:bold;padding: 8px 4px;',
								text: state
					}))).append($('<td/>',{
						style : 'border:1px solid #dfdfdf;font-weight:bold;',	
						align: 'center'				
				...
				$('input[name=refundBtn]').on('click',function(){
					var order_no =$(this).parent().prev().prev().prev().prev().text();
					var refundPop = window.open('/trading/refundForm.do?order_no='+order_no,'환불요청','width=455,height=455,resizable=no');
				});
