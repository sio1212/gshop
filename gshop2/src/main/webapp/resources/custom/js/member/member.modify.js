...
$(document).ready(function(){
	...
	$('input[name=textemail]').val($('#textemail').val());
...
$('#emailInput').change(function() {
	var textemail = $('#emailInput option:selected').val();
	$('#textemail').val(textemail);
	if($('#emailInput option:selected').val()=='') $('#textemail').attr('readonly',false);
	else $('#textemail').attr('readonly',true);
});
...
$('#pwdChangeBtn').click(function(){
	 var pwdPop = window.open('/member/changePwdForm.do','비밀번호 변경','width=400,height=400,resizable=no');		
});
...
$('#changeEmailBtn').click(function(){
	 var pwdPop = window.open('/member/changeEmailForm.do','이메일 변경','width=780,height=200,resizable=no');	
});
