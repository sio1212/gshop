...
function viewUnlimitedData(){
	$.ajax({
		type : 'post',
		url : '/admin/shop/getsalesInfoList.do',
		data : {
				'pg' : $('#pg').val(),
...
function viewlimitedData(){
	$.ajax({
		type : 'post',
		url : '/admin/shop/salesInfoSearch.do',
...
function salesInfoPaging(pg){
	location.href='/admin/shop/salesInfoList.do?pg='+pg;}
...
$('#openChart').click(function(){
	var chartPop = window.open('/admin/shop/salesChart.do','[Chart View]','width=565,height=530,resizable=no');
});
