...
function getOrderList(data){
	...
	if(data.userOrderList.length==0||data.userOrderList==null){
		$('#orderListTable tbody').append('<h5 style="color:red;font-weight:bold;text-decoration:underline;>주문 내역이 존재하지 않습니다</h5>');}
	else{
...
function orderPaging(pg){
	location.href='/admin/order/orderManage.do?pg='+pg;
}
...
function viewPopUp(order_no,order_state){
	var popup = window.open('/admin/order/personalOrderView.do?order_no='+order_no+'&order_state='+order_state,'개별주문서조회','width=785,height=1570,resizable=no');
}
...
$(document).ready(function(){
	$.ajax({
		type: 'post',
		url : '/admin/order/getUserOrderList.do',
		data: 'pg='+$('#pg').val(),
		dataType: 'json',
...
$('#orderSearchBtn').click(function(event,str){
	...
	if(keyword=='') 
		...
		$.ajax({
			type : 'post',
			url : '/admin/order/userSearchOrderList.do',
