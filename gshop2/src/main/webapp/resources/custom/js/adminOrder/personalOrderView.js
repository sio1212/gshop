...
$(document).ready(function(){
	...
	$.ajax({
		type: 'get',
		url: '/admin/order/getPersonalOrderInfo.do',
...
$('#doModifyDelivery').click(function(){
	if($('#sample2_postcode').val()==''||$('#sample2_address').val()==''||$('#sample2_detailAddress').val()==''||$('#sample2_extraAddress').val()=='') alert('주소를 올바르게 전부 입력해주세요');
		...
		$.ajax({
			type: 'post',
			url: '/admin/order/changeOrderInfo.do',
			data:{'order_address':'['+$('#sample2_postcode').val()+'] '+$('#sample2_address').val()+','+$('#sample2_detailAddress').val()+','+$('#sample2_extraAddress').val(),
...
$('#stateChangeBtn').click(function(){
	if(newState>2&&newState<8&&($('#order_deliverynum').val()==''||$('#order_deliverynum').val()=='[미정]')) alert('송장번호의 입력을 확인하고 먼저 주문서에 반영해주세요.');
		...
		if(realChange){
			$.ajax({
				type: 'post',
				url: '/admin/order/changeOrderState.do',
