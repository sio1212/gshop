...
function getReviewList(data){	
	...
	$.each(data.reviewList, function(index, items){			
		$('<tr/>').append($('<th/>',{
			...
			src : '/storage/showProduct.do?product_name_image='+items.productid,
			width :'100',
			height : '100'
		}))).append($('<td/>',{
			align : 'center',
	...
	$('#reviewTable').on('click','#subjectA',function(){
			...
			window.location='/board/review/reviewView.do?review_seq='+review_seq+'&pg='+$('#pg').val();
...
$(document).ready(function(){
	$.ajax({
		type : 'post',
		url : '/board/review/getReviewList.do',
		data : 'pg='+$('#pg').val(),
		dataType : 'json',
...
$('#reviewSearchBtn').click(function(event,str){
	...
	if($('#keyword').val()=='') 
		...
		$.ajax({
			type : 'post',
			url : '/board/review/reviewSearch.do',
...
function boardPaging(pg){
	location.href='/board/review/reviewList.do?pg='+pg;
}
