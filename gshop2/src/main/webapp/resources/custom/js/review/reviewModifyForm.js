	...
	$.ajax({
		...
		success : function(data){
			...
			if($("#productid option:selected").val()!=''){
				var getImgOn='<img style="height:80px;weight:60px;" src="/storage/showProduct.do?product_name_image='+$("#productid option:selected").val()+'">';
	...
	if($('#review_subject').val()=='') $('#missingMod').append(notitleMsg).alert();
		...
		$.ajax({
			...
			success : function(){
				...
				window.location='/board/review/reviewView.do?review_seq='+$('#review_seq').val()+'&pg='+$('#pg').val();}
		});
	}
});
