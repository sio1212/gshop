...
//2. 수정버튼 클릭시 비밀번호 입력란을 로그인시 보여주는 이벤트 
...
//3. 삭제버튼 클릭시 비밀번호 입력란을 로그인시 보여주는 이벤트
...
$('#checkReviewPwd').click(function(){
	...
	if($('#rePwd').val()==''){
		...
		if($('#purpose').val()=='modify'){
			window.location='/board/review/reviewModifyForm.do?review_seq='+$('#review_seq').val()+'&pg='+$('#pg').val();		
		}
		else if($('#purpose').val()=='delete'){
			var realDelete = confirm('정말 삭제하시겠습니까?');
			if(realDelete){
				window.location='/board/review/reviewDelete.do?review_seq='+$('#review_seq').val();}
		}
	}
	else{
