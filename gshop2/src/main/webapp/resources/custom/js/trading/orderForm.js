...
function setDeliveryFee(){
	if(deliveryPolicy!=null){
		...
		$('#deliveryFeeDiv').text(' 배송비 '+formatNumber(deliveryFee)+' 원 =');
		$('#subTotalDiv').text(' 합계: '+formatNumber(cartTotal+deliveryFee)+' 원');		
	}
}
...
function setDiscount(){
	...
	$('#finalTotal').text('총 '+formatNumber(finalTotal)+' 원').css('color','#008bcc').css('font-weight','bold');
}

// 주문 목록 셋팅
function setOrderList(orderList){
	$('#orderviewTable tr:gt(0)').empty();
	$.each(orderList, function(index, items){			
		...
		})).appendTo($('#orderviewTable tbody'));
	});	
		setDeliveryFee();
		$('#productTotalDiv').text('상품구매금액 '+formatNumber(cartTotal)+' 원 + ');
		$('#deliveryFeeDiv').text('배송비'+formatNumber(deliveryFee)+' 원 = ');
		$('#subTotalDiv').text(' 합계: '+formatNumber(cartTotal+deliveryFee)+' 원');		
...
$(document).ready(function(){
	...
	$.ajax({
		type: 'get',
		url: '/trading/getPreOrderInfo.do',
		dataType: 'json',
		success: function(data){
			...
			if(memberDTO!=null){
				...
				$('#order_email').val(memberDTO.email1+'@'+memberDTO.textemail);
				
				$('#sample2_postcode').val(memberDTO.sample2_postcode);

				$('#sample2_address').val(memberDTO.sample2_address);
				$('#sample2_detailAddress').val(memberDTO.sample2_detailAddress);
				$('#sample2_extraAddress').val(memberDTO.sample2_extraAddress);
...
$('#sample2_address').on('change',function(){
...
$('#choiceOrder').click(function(){
	if($('#order_name').val()==''||$('#order_receiver').val()==''||$('#order_tel').val()==''||$('#order_email').val()=='') alert('기본정보를 입력해주세요');
	...
	else if($('#sample2_postcode').val()==''||$('#sample2_address').val()==''||$('#sample2_detailAddress').val()==''||$('#sample2_extraAddress').val()=='') alert('배송지를 입력해주세요');
		...
		$.ajax({
			type: 'post',
			url : '/trading/putOrderForm.do',
			data : {'order_name':$('#order_name').val(),
					'order_receiver':$('#order_receiver').val(),
					'order_tel':$('#order_tel').val(),
					'order_email':$('#order_email').val(),
					'order_address':'['+$('#sample2_postcode').val()+'] '+$('#sample2_address').val()+','+$('#sample2_detailAddress').val()+','+$('#sample2_extraAddress').val(),
					'point':$('#point').val(),
			...
			success: function(data){
				if(data=='fail') {
					...
					window.location='/trading/orderView.do?order_no='+data;
				}
			}
		});
	}
