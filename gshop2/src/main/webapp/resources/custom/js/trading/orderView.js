...
$(document).ready(function(){
	...
	$.ajax({
		...
		success: function(data){
			...
			$('#order_ns').text(orderList.product_name_title);
...
$('#doModifyDelivery').click(function(){
	if($('#sample2_postcode').val()==''||$('#sample2_address').val()==''||$('#sample2_detailAddress').val()==''||$('#sample2_extraAddress').val()=='') alert('주소를 올바르게 전부 입력해주세요');
	else{
		$.ajax({
			type: 'post',
			url: '/trading/changeOrderInfo.do',
			data:{'order_address':'['+$('#sample2_postcode').val()+'] '+$('#sample2_address').val()+','+$('#sample2_detailAddress').val()+','+$('#sample2_extraAddressr').val(),
...
$('#requestRefundBtn').click(function(){
	var refundPop = window.open('/trading/refundForm.do?order_no='+$('input[name=order_no]').val(),'환불요청','width=455,height=455,resizable=no');
});
...
$('#deliveryConfirmBtn').click(function(){
	...
	if(realConfirm){
		$.ajax({
			type: 'get',
			url: '/trading/confirmDelivery.do',
			data: 'order_no='+$('input[name=order_no]').val(),
			success: function(){
...
$('#personalQABtn').click(function(){
	var personalQAPop = window.open('/member/personalQAForm.do','1:1문의','width=455,height=455,resizable=no');
});
